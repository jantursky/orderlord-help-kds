...
{% include img_tablet.html name="label_printer_setup_1.png" %}

- After clicking on this section, the new screen with be displayed. Click on the {% include icon.html name="settings" %} **Customization** category

{% include img_tablet.html name="label_printer_setup_2.png" %}
...
{% include img_tablet.html name="label_printer_setup_3.png" %}
...
- <span class="text-green-100">{% include icon.html name="check" %}</span> check if in the box of label printer was added also a USB cable
...
{% include img_tablet.html name="label_printer_how_to_setup_label_printer_usb_1.jpg" %}

{% include img_tablet.html name="label_printer_how_to_setup_label_printer_usb_3.jpg" %}
...
1. Once this option is enabled, a list of settings will be displayed. Click on the {% include icon.html name="phonelink_ring" %} **Connection type** option. 
...
1. If you successfully set up the USB Label printer, click on the {% include icon.html name="receipt" %} **Test print** category. If everything works, the label printer should till 10 seconds print label with _TEST MESSAGE_.
...
{% include img_tablet.html name="label_printer_how_to_setup_label_printer_usb_2.jpg" %}

## Wi-Fi Communication
Communication with a Wi-Fi connection with a Label printer is a bit harder for configuration. 

First of all, you need to know, if the label printer has available Wi-Fi (called Ethernet) connection. You could be sure in two ways:
1. Check the backside of the label printer. Be sure, that there is a port for **LAN cable**.
1. Try to do **the test print** of that label printer. Follow the steps for test print:
	...
	1. On this printer label configuration, there should be mentioned the line **"IP ADDRESS"** (or something similar describing IP address):
		1. If this line is mentioned there:
			1. and the IP address hasn't default value (mentioned in the 2. option) - the label printer should have already set up the IP address
			1. and the IP address is in the format **255.255.255.255**, **192.168.254.254**, **192.168.192.168**, **0.0.0.0** or similar - then you need to set up a new IP address for this printer
...
{% include img_tablet.html name="label_printer_how_to_setup_label_printer_wifi_1.jpg" %}

Once you know, that the label has enabled Wi-Fi port for communication, and you need to set up a new IP address of the Label printer, follow the steps:
...
- Open the application **Zebra setup utilities**, there should be an empty list of added label printers

{% include img_tablet.html name="label_printer_how_to_setup_label_printer_wifi_2.png" %}

- Click on the **Install New Printer** button and follow the new screen for adding a connected label printer. If the list already contains any printer (before or after installing a new printer), select this printer (specific one, where you want to configure), and click on the **Configure Printer Connectivity** button.

{% include img_tablet.html name="label_printer_how_to_setup_label_printer_wifi_3.png" %}

- A new dialog screen will be displayed, where select the option **Wireless**

{% include img_tablet.html name="label_printer_how_to_setup_label_printer_wifi_4.png" %}

- A new dialog screen will be displayed, select the option **Static** and setup **IP Address**, **Subnet mask**, and **Default gateway**. Type in the IP address that is free on the network, the subnet mask, and the default gateway. Please note that if you need to set up more printers, each has to have a unique IP address.
...
- the printer should restart itself with a newly assigned IP address. After the printer restarted, do a **Test print** and check if the printed test label with the configuration contains the new IP address.
	- <span class="text-green-100">{% include icon.html name="check" %}</span> if yes, **congratulations**, you set up the IP address for the label printer and you are able to use the label printer
...
{% include img_tablet.html name="label_printer_how_to_setup_label_printer_wifi_5.png" %}

- once everything was set up properly, you could try to connect with the label printer. Go to the **Label printer** category ([guide](#where-is-label-printer-category))
- click on the {% include icon.html name="print" %} **Enable label printing** option for {% include icon.html name="check_box" %}

{% include img_tablet.html name="label_printer_setup_4.png" %}

- once this option is enabled, a list of settings will be displayed. Click on the {% include icon.html name="phonelink_ring" %} **Connection type** option. 

{% include img_tablet.html name="label_printer_setup_5.png" %}

- select from the opened dialog option **Wi-Fi** and click on the <span class="text-green-200">**SET SELECTED**</span> button

{% include img_tablet.html name="label_printer_setup_6.png" %}

- you have 2 options on how to the setup IP address and port of the label printer:
- **automatic** - click on the {% include icon.html name="search" %} **Automatic printer discovery** row, a new dialog will be displayed and the application will try to find the Zebra printer in the same network as the Android device is connected. If the Zebra is listed in the list of founded devices, click on the device and IP address and port of the printer will be set up automatically

{% include img_tablet.html name="settings_label_printer_automatic_printer_discovery_3.png" %}

- **manual** - click on the {% include icon.html name="label" %} **The IP address of the printer** row, a new dialog will be displayed, type there assigned IP of the printer (for example **"192.168.1.30"**) and click on the <span class="text-green-200">**SET SELECTED**</span> button. Then click on the {% include icon.html name="label" %} **TCP port of the printer** row, another dialog will be displayed, type there a port of the printer (the default is **"9100"** - try to use this value) and click on <span class="text-green-200">**SET SELECTED**</span> button.

{% include img_tablet.html name="label_printer_setup_8.png" %}
